...
try:
    ...
    print('The cryptography package is not installed'
          '\nPlease run `pip install -r requirements.txt` to install')
    raise
...
def encrypt(args, count=0, keys_dir=None):
    '''
    ...
    # Generate encrypted NVS Partition
    generate(args, is_encr_enabled=True, encr_key=key, count=count, keys_dir=keys_dir)
...
def generate_key(args, keys_dir=None):
    '''
...
def generate(args, is_encr_enabled=False, encr_key=None, count=0, keys_dir=None):
    '''
    ...
    args.outdir, args.output = set_target_filepath(args.outdir, args.output)
    bin_output_dir =  os.path.dirname(args.output)

    if is_encr_enabled and not encr_key:
        encr_key = generate_key(args, keys_dir=keys_dir)
    ...
    with open(args.input, 'rt', encoding='utf8') as input_file,\
        ...
        while True:
            ...
            try:
                # Check key length
                if len(data["key"]) > 15:
                    raise InputError("Length of key `%s` should be <= 15 characters." % data["key"])
    ...
    if count % step_cnt == 0:
        ...
        print("\nCreating NVS binary with version:", version_set)
        print("\n[{:<6}]NVS binary generated: {} ===> {}".format(timestamp, str(count), bin_output_dir))
