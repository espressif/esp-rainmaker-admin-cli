...
try:
    ...
    sys.exit(1)
try:
    from tools import nvs_partition_gen
...
def verify_values_exist(input_values_file, values_file_data, key_count_in_values_file, line_no=1):
    ...
    if len(values_file_data) != key_count_in_values_file:
       ...
       Count is greater than the values in files provided in config."
...
def create_intermediate_csv(args, keys_in_config_file, keys_in_values_file, keys_repeat, is_encr=False):
    ...
    try:
        ...
        while values_data_line:
            ...
            output_dir_str = os.path.join('node_details', file_identifier_value)
            output_csv_target_dir = create_dir(output_dir_str, args.outdir)
            csv_filename = "node_info" + "." + csv_str
            ...
            if set_output_keyfile:
                args.keyfile = "node_encr_key.bin"

            if is_encr:
                encr_key_dir_str = os.path.join(args.outdir, 'node_details', file_identifier_value)
                nvs_partition_gen.encrypt(args, count=cnt, keys_dir=encr_key_dir_str)
            else:
                nvs_partition_gen.generate(args, count=cnt, keys_dir=output_dir_str)
        ...
        print("\nTotal NVS binary generated: {}".format(str(cnt - 1)))
        path_sep = os.sep
        print("\nBinary files saved at location {}{}bin".format(args.outdir.rstrip(path_sep), path_sep))
...
def generate(args, create_csv=False):
    keys_in_config_file = []
    keys_in_values_file = []
    keys_repeat = []
